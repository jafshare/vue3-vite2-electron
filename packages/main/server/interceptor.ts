...
export default {
  ...
  *beforeSendRequest(requestDetail: RequestDetail) {
    logger.log("发送http请求:", requestDetail.url);
  ...
  *beforeDealHttpsRequest(requestDetail: RequestDetail) {
    logger.log("发送https请求:", requestDetail.url);
    yield new Promise((resolve, reject) => {
  ...
  *onError(requestDetail: RequestDetail, error: Error) {
    logger.log("发送http请求错误:", requestDetail.url, error);
    yield new Promise((resolve, reject) => {
      resolve(null);
    });
  },
  ...
  *onConnectError(requestDetail: RequestDetail, error: Error) {
    logger.log("发送https请求错误:", requestDetail.url, error);
    yield;
  },
};
